...
class UserManager {
 public:
  UserManager() { std::cout << "[UserManager] Initialized - will publish UserLoginEvent" << std::endl; }
...
class OrderProcessor {
 public:
  ...
  void processOrder(const std::string& customer, double amount) {
    ...
    std::cout << "\n[OrderProcessor] Processing order #" << order_id << " for '" << customer << "' - $" << amount
              << std::endl;
...
class SystemMonitor {
 public:
  SystemMonitor() { std::cout << "[SystemMonitor] Initialized - will publish SystemAlertEvent" << std::endl; }
...
class EventLogger {
 public:
  EventLogger() {
    ...
    login_subscription_ =
        getSyncEventBus().subscribe<UserLoginEvent>([this](const UserLoginEvent& event) { logUserEvent(event); });

    order_subscription_ =
        getSyncEventBus().subscribe<OrderPlacedEvent>([this](const OrderPlacedEvent& event) { logOrderEvent(event); });

    alert_subscription_ =
        getSyncEventBus().subscribe<SystemAlertEvent>([this](const SystemAlertEvent& event) { logAlertEvent(event); });

    std::cout << "[EventLogger] Successfully subscribed to all event types" << std::endl;
  }

  void logUserEvent(const UserLoginEvent& event) {
    std::cout << "  [EVENT_LOG] User Event: '" << event.username << "' from IP " << event.ip_address << std::endl;
    std::cout << "    -> Logged to user activity audit trail" << std::endl;
    user_events_logged_++;
  }

  void logOrderEvent(const OrderPlacedEvent& event) {
    std::cout << "  [EVENT_LOG] Order Event: #" << event.order_id << " by '" << event.customer << "' for $"
              << event.amount << std::endl;
  ...
  void logAlertEvent(const SystemAlertEvent& event) {
    std::cout << "  [EVENT_LOG] System Alert [" << event.level << "]: " << event.message << std::endl;
  ...
  void printStatistics() {
    ...
    std::cout << "  - Total events processed: " << (user_events_logged_ + order_events_logged_ + alert_events_logged_)
              << std::endl;
  }
