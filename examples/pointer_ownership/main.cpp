...
class DataProcessor {
 public:
  DataProcessor() { std::cout << "[DataProcessor] Initialized" << std::endl; }
  ...
  void processData(const std::string& input) {
    ...
    DataProcessedEvent event{"text_processing", result, std::chrono::system_clock::now()};
    ...
    std::cout << "[DataProcessor] Original processor still has access to result: " << *result << std::endl;
  }
};

class ResourceManager {
 public:
  ResourceManager() : next_id_(1000) { std::cout << "[ResourceManager] Initialized" << std::endl; }
  ...
  void allocateResource(const std::string& type) {
    ...
    ResourceAllocatedEvent event{type, std::move(resource_id), "ResourceManager"};
...
class NotificationService {
 public:
  ...
  void sendNotification(const std::string& title, const std::string& message) {
    ...
    NotificationEvent event{title, message, &service_name_};
 ...
 public:
  AnalyticsService() {
    ...
    data_subscription_ = getPointerEventBus().subscribe<DataProcessedEvent>(
        [this](const DataProcessedEvent& event) { handleDataProcessed(event); });
  }
 ...
 public:
  AlertHandler() {
    ...
    notification_subscription_ = getPointerEventBus().subscribe<NotificationEvent>(
        [this](const NotificationEvent& event) { handleNotification(event); });
  }
