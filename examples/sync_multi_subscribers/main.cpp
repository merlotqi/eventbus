...
using SyncEventBus = eventbus::EventBus<
    eventbus::Synchronous,  // Synchronous execution - blocking, single-threaded
...
public:
    SecurityMonitor(SyncEventBus& bus) : bus_(bus) {
        ...
        login_subscription_ = bus_.subscribe<UserLoginEvent>(
    ...
    void handleLogin(const UserLoginEvent& event) {
        std::cout << "  [SecurityMonitor] User '" << event.username
                  << "' logged in from " << event.ip_address << std::endl;
...
public:
    OrderProcessor(SyncEventBus& bus) : bus_(bus) {
        ...
        order_subscription_ = bus_.subscribe<OrderPlacedEvent>(
    ...
    void handleOrder(const OrderPlacedEvent& event) {
        std::cout << "  [OrderProcessor] Processing order #" << event.order_id
                  << " for customer '" << event.customer << "'" << std::endl;
...
public:
    SystemAdministrator(SyncEventBus& bus) : bus_(bus) {
        ...
        alert_subscription_ = bus_.subscribe<SystemAlertEvent>(
    ...
    void handleLogin(const UserLoginEvent& event) {
        std::cout << "  [SystemAdministrator] User '" << event.username
                  << "' login monitored - logging to audit trail" << std::endl;
    }

    void handleAlert(const SystemAlertEvent& event) {
        std::cout << "  [SystemAdministrator] ALERT [" << event.level << "]: "
