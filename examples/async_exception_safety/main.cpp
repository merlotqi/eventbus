...
int main() {
  using AsyncBus =
      eventbus::EventBus<eventbus::Asynchronous, eventbus::UnboundedQueue, eventbus::BlockProducer, TestEvent>;

  AsyncBus bus;

  // Subscribe multiple handlers, some will throw exceptions
  auto sub1 = bus.subscribe<TestEvent>(
      [](const TestEvent& e) { std::cout << "Handler 1: Processing '" << e.message << "' - SUCCESS" << std::endl; });
  ...
  auto sub3 = bus.subscribe<TestEvent>(
      [](const TestEvent& e) { std::cout << "Handler 3: Processing '" << e.message << "' - SUCCESS" << std::endl; });
  ...
  auto sub5 = bus.subscribe<TestEvent>(
      [](const TestEvent& e) { std::cout << "Handler 5: Processing '" << e.message << "' - SUCCESS" << std::endl; });
